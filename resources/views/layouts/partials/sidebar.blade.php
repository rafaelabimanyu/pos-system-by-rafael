{{-- Sidebar Navigation --}}
<aside id="sidebar"
    class="fixed top-0 left-0 z-50 w-64 h-screen bg-white border-r border-slate-200 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col shadow-sm">

    {{-- Logo Section --}}
    <div class="flex items-center gap-3 px-5 h-16 border-b border-slate-200 shrink-0 bg-white">
        <!-- Gambar Logo Baru -->
        <a href="/" class="shrink-0">
            <!-- Ubah bagian src menjadi seperti di bawah ini -->
            <img src="{{ asset('logo/image.png') }}" alt="Logo" class="h-10 w-auto object-contain">
        </a>

        <!-- Teks Tiysa POS & Point of Sale (Dipertahankan) -->
        <div>
            <h1 class="text-lg font-bold text-slate-900 tracking-tight">
                {{ \App\Models\Setting::get('store_name', 'Tiysa POS') }}</h1>
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto bg-white">
        ...
        @if (auth()->user()->isAdmin())
            <a href="{{ route('dashboard') }}"
                ...
                <i data-lucide="layout-dashboard"
                    class="sidebar-icon w-5 h-5 {{ request()->routeIs('dashboard') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }} transition-colors"></i>
        ...
        <a href="{{ route('pos') }}"
            ...
            <i data-lucide="monitor"
                class="sidebar-icon w-5 h-5 {{ request()->routeIs('pos') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }} transition-colors"></i>
        ...
        @if (auth()->user()->isAdmin())
            <a href="{{ route('produk.index') }}"
                ...
                <i data-lucide="package"
                    class="sidebar-icon w-5 h-5 {{ request()->routeIs('produk.*') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }} transition-colors"></i>
                <span class="sidebar-text transition-colors">Produk</span>
            </a>

            {{-- Reports --}}
            <p class="px-3 mt-6 mb-2 text-[11px] font-semibold text-slate-500 uppercase tracking-widest">Laporan & Data
            </p>
            ...
            <a href="{{ route('laporan') }}"
                ...
                <i data-lucide="bar-chart-3"
                    class="sidebar-icon w-5 h-5 {{ request()->routeIs('laporan') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }} transition-colors"></i>
            ...
            <a href="{{ route('users') }}"
                ...
                <i data-lucide="users"
                    class="sidebar-icon w-5 h-5 {{ request()->routeIs('users') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }} transition-colors"></i>
            ...
            <a href="{{ route('pengaturan') }}"
                ...
                <i data-lucide="settings"
                    class="sidebar-icon w-5 h-5 {{ request()->routeIs('pengaturan') ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-500' }} transition-colors"></i>
